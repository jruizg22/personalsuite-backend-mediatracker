...
from media_tracker.services import media_translation_service
...
def get_router(get_session: Callable[[], Generator[Session, Any, None]]) -> APIRouter:
    ...
    def get_media_by_id(
        ...
        Args:
            ...
            session (Session): Database session dependency.
            view (MediaTranslationView): Level of detail for the returned media.
        ...
        """
        try:
            return media_translation_service.get_by_id(session, media_translation_id, view)
    ...
    def create_media(new_media_translation: MediaTranslationCreate, session: Session = Depends(get_session)) -> MediaTranslationPublic:
        """
    ...
    def update_media(media_translation_id: int, media_translation_in: MediaTranslationUpdate, session: Session = Depends(get_session)) -> MediaTranslationPublic:
        """
        ...
        """
        try:
            return media_translation_service.update(session, media_translation_id, media_translation_in)
        except ResourceNotFoundError as e:
            raise HTTPException(status_code=404, detail=(f"Resource not found: {e}"))
        except Exception as e:
            raise HTTPException(status_code=500, detail=(f"Error updating media translation with ID {media_translation_id}: {e}"))

    @router.delete("/{media_translation_id}", status_code=204)
    def delete_media(media_translation_id: int, session: Session = Depends(get_session)) -> None:
        """
        ...
        """
        try:
            media_translation_service.delete(session, media_translation_id)
        except ResourceNotFoundError as e:
            raise HTTPException(status_code=404, detail=(f"Resource not found: {e}"))
        except Exception as e:
            raise HTTPException(status_code=500, detail=(f"Error deleting media with ID {media_translation_id}: {e}"))

...
from media_tracker.models.yt import YTChannel, YTChannelPublic, YTChannelCreate, YTChannelUpdate


def get_all(session: Session, offset: int = 0, limit: int = 100) -> list[YTChannelPublic]:
    try:
        yt_channel_db = session.exec(select(YTChannel).offset(offset).limit(limit)).all()
